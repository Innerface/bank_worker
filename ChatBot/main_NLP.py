...
extrapair = extractSOPair.extractSOPair()
mysql = Mysql()
...
def run_QA(session, ques, id, operationID=0, flag=0, label=None):
    ...
    new_SO_pair = extrapair.markOneQ(ques)
    print(new_SO_pair)

    # 利用translator进行pair的更新
    CorruptionFlag = False
    if not new_SO_pair:
        CorruptionFlag = True

        # new_SO_pair = decode_pair.decode_so_pair(ques)
        # with tf.Session() as sess:
        #     model = decode_pair.create_model(sess, True)
        #     model.batch_size = 1
        #     # query_test = '金卡、白金卡和普卡有什么区别'
        #     new_SO_pair = decode_pair.decode(model,ques)
    ...
    # same_sen = Mysql()
    sqlAll = "select id,type,sentence,rel_sentence_id from synonym_sentence where sentence = '%s'" % ques
    result = mysql.getAll(sqlAll)
    # same_sen.dispose()
    if result:
        if result[0]['type'] == 1:
            sqlAll = "select answer from question_t where question = '%s'" % ques

            result_answer = mysql.getAll(sqlAll)
            print(result_answer)
            # same_sen.dispose()
            answer_value = {'answer_value': [result_answer[0]['answer'].decode('utf-8')],
                            'recommend_answer': [],
                            'recommend_question': [],
                            'SO_pair': new_SO_pair}
            return answer_value
        else:
            sqlAll = "select sentence from synonym_sentence where id = %s" % result[0]['rel_sentence_id']
            result_an = mysql.getAll(sqlAll)
            # same_sen.dispose()
            sqlAll = "select answer from question_t where question = '%s'" % result_an[0]['sentence'].decode('utf-8')
            result_ans = mysql.getAll(sqlAll)
            # same_sen.dispose()
            answer_value = {'answer_value': [result_ans[0]['answer'].decode('utf-8')],
                            'recommend_answer': [],
                            'recommend_question': [],
                            'SO_pair': new_SO_pair}
            return answer_value
    ...
    if value == None:
        ...
        question = ques
        test = QAstream(str(question), new_SO_pair, CorruptionFlag)
        queryPair = [None, None]
        # 需要每次问答都更新的量：
        initial_parameter2 = {'Query_Raw': test.Query_Raw, 'Query_Parsed': test.Query_Parsed, 'SO_pair': new_SO_pair,
        ...
        if classify_result == 1 or label or operationID > 0:
            ...
            if flag == 0:
                answer_value = {}
                if '人工接管' in NewQuery or '人工' in NewQuery:
                    answer_rg = QA_answer(session, NewQuery, stdstream, CorruptionFlag, SO_pair, simscore, Query_Raw)
                    answer_value = {'flag': 3,
                                    'answer_value': answer_rg['answer_value'],
                                    'recommend_answer': answer_rg['recommend_answer'],
                                    'recommend_question': answer_rg['recommend_question'],
                                    'SO_pair': queryPair}
                    return answer_value
            ...
            if flag == 1:
                answer_value = {'flag': 1,
                                'recommend_question':
                                    QA_answer(session, NewQuery, stdstream, CorruptionFlag, SO_pair, simscore,
                                              Query_Raw)[
                                        'recommend_question'],
                                'SO_pair': queryPair}
                return answer_value
...
def seg_pair(ques):
    ...
    sen_seg = remove_stop_word(sen_seg)
    sen_seg = '|'.join(sen_seg)
    sen_so_pair = extrapair.markOneQ(ques)
    return sen_seg, sen_so_pair


if __name__ == "__main__":
    que = '3001_12_34HJK51+workflow'
    res = maintf("金卡额度是多少", que, operationID=0, flag=0, label=[])
    print(res)
    res = maintf('信用卡余额', que, operationID=30, label=['bank_card'])
    print(res)
    # res = maintf('我要办理信用卡，怎么办', que)
    # print(res)
    # res = maintf("金卡额度是多少", que)
    # print(res)
    # res = maintf("什么是活期存款", que)
    # print(res)
    # res = maintf("贵金属", que)
    # print(res)
    # res = maintf("储蓄卡和信用卡有什么区别", que)
    # print(res)
    # res = maintf("你好", que)
    # print(res)
    # res = maintf("金卡、白金卡的额度一样吗", que)
    # print(res)
    # res = maintf("什么是零存整取", que)
    # print(res)
